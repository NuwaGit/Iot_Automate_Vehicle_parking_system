...
class CameraHandler:
    ...
    def __init__(self, camera_index=0, use_config=True, credentials_path=None):
        """
        ...
        self._init_vision_client(credentials_path)
    ...
    def _init_vision_client(self, credentials_path=None):
        """
        Initialize Google Cloud Vision client
        ...
        try:
            ...
            if credentials_path:
                ...
                # Try default location in raspberry_pi directory
                script_dir = os.path.dirname(os.path.abspath(__file__))
                default_creds = os.path.join(script_dir, 'google_credentials.json')
                if os.path.exists(default_creds):
                    os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = default_creds
                    logger.info(f"Using default Google Cloud credentials: {default_creds}")
                else:
                    logger.warning("No Google Cloud credentials found. OCR may fail.")
    ...
    def extract_number_plate(self, image: np.ndarray) -> Optional[str]:
        """
        Extract number plate text from image using Google Cloud Vision API
        ...
        try:
            ...
            # Encode image to bytes for Google Vision API
            ...
            if texts:
                ...
                cleaned_text = self._clean_number_plate_text(raw_text)
                
                if cleaned_text:
                    logger.info(f"Extracted number plate: {cleaned_text}")
                    return cleaned_text
                else:
                    logger.warning("No valid number plate text extracted")
                    return None
            else:
